...
class Marble:
	...
	def connect(self,bustype=None,server=None,port=None,password=None,channel=None):
		try:
			...
			self.info.comm.bustype = bustype
			self.info.comm.server = server
			self.info.comm.port = port
			self.info.comm.password = password
			self.info.comm.channel = channel
			self.info.comm.__init__()
